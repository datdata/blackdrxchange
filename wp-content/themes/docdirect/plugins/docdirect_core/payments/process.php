...
if ( isset( $_POST['payment_status'] ) && $_POST['payment_status'] == 'Completed' ) {
   ...
   if (strcmp ($res, "VERIFIED") == 0) {
			...
			if( !empty( $userdata->user_email ) ) {
				...
				$user_email	= $_POST['receiver_email'];
			}
			...
			if( class_exists( 'DocDirectProcessEmail' ) ) {
				...
				$emailData['invoice']	  	   = $_POST['txn_id'];
				$emailData['package_name']	  = $_POST['item_name'];					
				$emailData['amount']			= $_POST['mc_currency'].$_POST['mc_gross'];
...
if ( isset( $_POST['x_response_code'] ) && $_POST['x_response_code'] == '1' ) {
	...
	if( isset( $user_name ) && !empty( $user_name ) ) {
		...
		$user_name	= $_POST['x_first_name'] . ' ' . $_POST['x_last_name'];
	}
	...
	if( class_exists( 'DocDirectProcessEmail' ) ) {
		...
		$emailData	= array();
		$emailData['mail_to']	  	   = $user_email;
		$emailData['name']			  = $_POST['x_first_name'].' '.$_POST['x_last_name'];
		$emailData['invoice']	  	   = $_POST['x_trans_id'];
		$emailData['package_name']	  = $_POST['x_description'];					
		$emailData['amount']			= isset( $mc_currency ) ? $mc_currency :'USD'.$_POST['x_amount'];
		$emailData['status']			= esc_html__('Approved','docdirect_core');
