...
if ( ! function_exists( 'docdirect_update_service_category' ) ) {
	function docdirect_update_service_category(){ 
		...
		$services_cats	= array();
		$key	 = !empty( $_POST['key'] ) ? $_POST['key'] : 'new';
		$type	= !empty( $_POST['type'] ) ? $_POST['type'] : 'add';
...
if ( ! function_exists( 'docdirect_delete_service' ) ) {
	function docdirect_delete_service(){ 
		...
		unset( $booking_services[$_POST['key']] );
...
if ( ! function_exists( 'docdirect_add_time_slots' ) ) {
	function docdirect_add_time_slots(){ 
		...
		//$slot_title	  = !empty( $slot_title ) ? $slot_title : docdirect_unique_increment(5);
...
if ( ! function_exists( 'docdirect_save_custom_slots' ) ) {
	function docdirect_save_custom_slots(){
		...
		$custom_timeslots_object = stripslashes($_POST['custom_timeslots_object']);
...
if ( ! function_exists( 'docdirect_add_custom_time_slots' ) ) {
	function docdirect_add_custom_time_slots(){ 
		...
		$current_times = json_decode(stripslashes($_POST['custom_time_slots']),true);
		$current_times_details = !empty($_POST['custom_time_slot_details']) ? json_decode(stripslashes($_POST['custom_time_slot_details']),true) : array();
...
if ( ! function_exists( 'docdirect_get_time_slots_list' ) ) {
	function docdirect_get_time_slots_list(){ 
		global $current_user, $wp_roles,$userdata,$post;
		$user_identity	= $current_user->ID;
		$timeslots = json_decode(stripslashes($_POST['json_list']),true);
...
if ( ! function_exists( 'docdirect_delete_custom_time_slots' ) ) {
	function docdirect_delete_custom_time_slots(){ 
		...
		if( !empty( $_POST['slot'] )
			&&
			!empty( $_POST['current_slots'] )	
		){
			$delete_slot = $_POST['slot'];
			$current_slots = json_decode(stripslashes($_POST['current_slots']),true);
			$current_slot_details = json_decode(stripslashes($_POST['current_slot_details']),true);
