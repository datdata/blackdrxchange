				...
				<div class="comment tg-border">
					<figure class="tg-author-img">
						<a href="<?php echo esc_url($user_url); ?>">
							<?php
								if (isset($userprofile_media) && !empty($userprofile_media) && $userprofile_media != '') {
									echo '<img src="' . esc_url($userprofile_media) . '" alt="'.esc_attr__('Author Avatar','docdirect').'">';
								} else {
									echo get_avatar($user_id, 100,'','',array('height'=>89,'width'=>140));
								}
							?>
						</a>
					</figure>
    ...
    function format_comment($comment, $args, $depth) {
        ...
        <li>
			...
			<?php if ($comment->comment_approved == '0') : ?>
				<em><?php esc_html_e('Your comment is awaiting moderation.', 'docdirect') ?></em><br />
			<?php endif;
		}
...
<div class="tg-comment-formarea tg-haslayout">
	<div class="row">
		...
		comment_form(array(
			...
			'</div></div>',
			'logged_in_as' => '<div class="col-xs-12"><div class="form-group">' . sprintf('Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', admin_url('profile.php'), $user_identity, wp_logout_url(apply_filters('the_permalink', get_permalink()))) . '</div></div>',
			'notes' => '',
			...
			'title_reply_before' => '<div class="col-xs-12"><div class="tg-heading-border tg-small"><h3>get in touch',
